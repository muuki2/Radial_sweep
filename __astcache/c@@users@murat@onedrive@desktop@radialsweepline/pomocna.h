...
struct Tacka {
	...
	bool operator<(Tacka) const;
	bool operator==(Tacka) const;
	double operator-(Tacka) const;
...
struct Duz {
	...
	void Crtaj(TImage*, TColor = clBlack, int velicina = 3) const;
	bool horizontalna() const;
	bool vertikalna() const;
};

struct Event {
    double angle;
    int type; // 1 za početak, -1 za kraj
    Duz segment;
    bool operator<(const Event& other) const {
        if (fabs(angle - other.angle) > 1e-9)
...
struct DuziPoY {
    bool operator()(const Duz* d1, const Duz *d2) const {
        if (d1->A == d2->A) {
