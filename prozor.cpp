...
bool TForm1::je_vidljiva(const Tacka& A, const Duz& duz, const set<const Duz*, DuziPoY>& aktivne_duzi)
{
    Duz zraka(A, duz.A);
    for (const auto& aktivna_duz : aktivne_duzi) {
        if (aktivna_duz->A == duz.A || aktivna_duz->B == duz.A) continue;
        if (duzi_se_sijeku(zraka, *aktivna_duz)) {
            return false;
        }
    }
    return true;
...
bool TForm1::duzi_se_sijeku(const Duz& duz1, const Duz& duz2)
{
    Tacka a = duz1.A, b = duz1.B, c = duz2.A, d = duz2.B;
    double orijentacija1 = Orijentacija(a, b, c);
    double orijentacija2 = Orijentacija(a, b, d);
    double orijentacija3 = Orijentacija(c, d, a);
    double orijentacija4 = Orijentacija(c, d, b);
    if (((orijentacija1 > 0 && orijentacija2 < 0) || (orijentacija1 < 0 && orijentacija2 > 0)) &&
        ((orijentacija3 > 0 && orijentacija4 < 0) || (orijentacija3 < 0 && orijentacija4 > 0)))
        return true;
    return tacka_na_duzi(a, b, c) || tacka_na_duzi(a, b, d) ||
           tacka_na_duzi(c, d, a) || tacka_na_duzi(c, d, b);
}

bool TForm1::tacka_na_duzi(const Tacka& a, const Tacka& b, const Tacka& c)
{
	const double EPS = 1e-9;
	return fabs(Orijentacija(a, b, c)) < EPS &&
		   min(a.x, b.x) <= c.x && c.x <= max(a.x, b.x) &&
		   min(a.y, b.y) <= c.y && c.y <= max(a.y, b.y);
}

void __fastcall TForm1::ButtonVidljiveDuziClick(TObject *Sender)
{
    Tacka A(0, 0); // Točka iz koje gledamo (0,0)
    vector<Duz> vidljive_duzi;
    vector<Duz> nevidljive_duzi;

    // Pretvaramo postojeće duži u događaje
    vector<Event> eventi;
    for (const auto& duz : duzi) {
        double kut1 = atan2(duz.A.y, duz.A.x);
        double kut2 = atan2(duz.B.y, duz.B.x);
        if (kut1 > kut2) swap(kut1, kut2);
        eventi.push_back({kut1, 1, duz});
        eventi.push_back({kut2, -1, duz});
    }
    sort(eventi.begin(), eventi.end());

    set<const Duz*, DuziPoY> aktivne_duzi;

    for (const auto& event : eventi) {
        if (event.type == 1) { // početak duži
            if (je_vidljiva(A, event.segment, aktivne_duzi)) {
                vidljive_duzi.push_back(event.segment);
                event.segment.Crtaj(Slika, clRed);
            } else {
                nevidljive_duzi.push_back(event.segment);
            }
            aktivne_duzi.insert(&event.segment);
        } else { // kraj duži
            aktivne_duzi.erase(&event.segment);
        }
    }

   ShowMessage("Broj vidljivih duži: " + IntToStr(static_cast<int>(vidljive_duzi.size())));
	vidljive_duzi.clear();
	nevidljive_duzi.clear();
	eventi.clear();
	aktivne_duzi.clear();
}
