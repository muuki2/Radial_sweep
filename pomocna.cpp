...
bool Tacka::operator<(Tacka t2) const {
	if (this->x == t2.x)
		return this->y < t2.y;
	return this->x < t2.x;
}
...
double Tacka::operator-(Tacka t2) const{
...
void Duz::Crtaj(TImage* Slika, TColor boja, int velicina) const  {
	Slika->Canvas->Brush->Color=boja;
...
bool Duz::horizontalna() const {
	return this->A.y == this->B.y;
}

bool Duz::vertikalna() const {
